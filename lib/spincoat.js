...
export function polygonIntervalsAtY(shapes, layer, y) {
  ...
  for (const shape of shapes) {
    ...
    if (shape.points.some((point) => point.y === y)) touchesBoundary = true;
    const crossings = [];
  ...
  return { intervals: mergeIntervals(intervals), ignoredPaths, touchesBoundary };
}
...
export function sampleIntervals(intervals, minX, maxX, count) {
  ...
  const cellWidth = (maxX - minX) / count;
  const merged = mergeIntervals(intervals);
  return Array.from({ length: count }, (_, index) => {
    const cellStart = minX + index * cellWidth;
    const cellEnd = cellStart + cellWidth;
    const coveredWidth = merged.reduce((sum, [start, end]) => (
      sum + Math.max(0, Math.min(cellEnd, end) - Math.max(cellStart, start))
    ), 0);
    return Math.max(0, Math.min(1, coveredWidth / cellWidth));
  });
}
...
export function buildMaterialColumns({ count, substrate, layers }) {
  ...
  for (const layer of layers) {
    ...
    columns.forEach((column, index) => {
      const coverage = layer.mode === "uniform"
        ? 1
        : Math.max(0, Math.min(1, Number(layer.mask[index]) || 0));
      if (layer.mode === "etch") {
        if (coverage > 0) etchColumn(column, thicknessNm * coverage);
        return;
      }
      if (layer.mode === "patterned" && coverage <= 0) return;
      const bottom = column.at(-1)?.top ?? -substrateThicknessNm;
      column.push({ name: layer.name, color: layer.color, bottom, top: bottom + thicknessNm * coverage });
    });
  }
  return columns;
}
