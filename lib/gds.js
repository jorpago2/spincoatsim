...
export function flattenGds(model, topCell) {
  ...
  function visit(name, matrix, stack) {
    ...
    if (stack.includes(name)) throw new Error(`Circular GDS reference: ${[...stack, name].join(" â†’ ")}.`);
    const structure = model.structures.get(name);
    if (!structure) throw new Error(`Referenced cell â€œ${name}â€ does not exist.`);
  ...
  if (!output.length) throw new Error(`Cell â€œ${topCell}â€ contains no rasterizable BOUNDARY, BOX or PATH elements.`);
  return output;
}
